...
class DriveToCoord(Node):
    ...
    def emergencystop(self):
        self.get_logger().error("🚨 NOODSTOP  GOAL ANNULEREN + CMD_VEL STOP")

        if self._goal_handle is not None:  # <--- DIT IS DE CRUCIALE CHECK
                try:
                    self._goal_handle.cancel_goal_async()
                    self.get_logger().error("🚨 Actieve goal geannuleerd door noodstop")
                except Exception as e:
                    self.get_logger().warn(f"Cancel goal fout: {e}")
                self._goal_handle = None
        else:
            # Als er geen goal is, hoeven we alleen de motoren te stoppen
            self.get_logger().info("Noodstop: Geen actieve goal om te annuleren.")


# 1️⃣ Cancel eigen goal
...
#           self.currentgoal = None
        ...
        # 2️⃣ Kleine gecontroleerde achteruit beweging
        #back_msg = Twist()
        #back_msg.linear.x = -0.05   # 5 cm/s achteruit
...
#       for i in range(10):
#           self.cmd_vel_pub.publish(stop_msg)
#           time.sleep(0.05) # ROBBE WIJZEGING 18 FEB
    ...
    def goal_response_callback(self, future):
        # Veilig ophalen van het GoalHandle — voorkom racecondities en None waarden
        try:
            goal_handle = future.result()
        except Exception as e:
            self.get_logger().error(f"Fout bij goal_response_future: {e}")
            return

        if goal_handle is None:
            self.get_logger().warn("Goal response futurereturned None — geen goal handle beschikbaar.")
            return

        # Bewaar voor eventuele annulering/boekhouding, maar gebruik lokale variabele voor vervolgacties
        ...
        # Gebruik lokale variabele om te voorkomen dat een andere callback (bv. cancel)
        # self._goal_handle op None zet vóórdat we get_result_async aanroepen.
        get_result_future = goal_handle.get_result_async()
        get_result_future.add_done_callback(self.result_callback)
        self._get_result_future = get_result_future
