...
$SETUP_CMD &&
echo '📷 Camera gestart (automatische herstart actief)'

while true; do
    echo '▶️ Orbbec camera wordt gestart...'

    ros2 launch turn_on_wheeltec_robot wheeltec_camera.launch.py
    EXIT_CODE=\$?

    if [ \$EXIT_CODE -eq 0 ]; then
        echo '⚠️ Camera launch is gestopt.'
    elif [ \$EXIT_CODE -eq 130 ]; then
        echo '🛑 Camera gestopt door SIGINT — geen autorestart'
        break
    else
        echo \"⚠️ Camera crashte met exit-code \$EXIT_CODE\"
    fi

    echo '🔄 Camera wordt opnieuw gestart in 3 seconden...'
    sleep 3
done
...
start_visualtracker() {
    gnome-terminal --title="People follower" -- bash -c "
        $SETUP_CMD && \
        ros2 run people_follower_ros2 visualtracker \
        $(log_cmd visualtracker.log)
    "
}
